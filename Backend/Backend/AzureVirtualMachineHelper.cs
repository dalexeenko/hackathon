...
namespace GetSubscriptionInfo
{
    ...
    public static class AzureVirtualMachineHelper
    {
        public static async Task<IEnumerable<AzureVmInfo>> ListVirtualMachines(
            ...
            string version)
        {
            ...
            var cloudServices = await ListCloudServices(subscriptionId, certificate, version);

            foreach (var service in cloudServices)
            {
                var vms = await ListVms(subscriptionId, service.Name, certificate, version);
        ...
        private static async Task<IEnumerable<AzureCloudService>> ListCloudServices(
            string subscriptionId,
            X509Certificate2 certificate,
            string version)
        {
        ...
        private static async Task<IEnumerable<AzureVmInfo>> ListVms(
            string subscriptionId,
